...
const ProductDetail = () => {
  ...
  const handleWishlistToggle = useCallback(async () => {
    ...
    if (user?.role !== 'customer') {
      toast({
        ...
        description: t('onlyCustomersWishlist'),
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: added ? t('addedToWishlist') : t('removedFromWishlist'),
        description: added
          ? t('productAddedToWishlist', { name: product.name ?? 'Product' })
          : t('productRemovedFromWishlist', { name: product.name ?? 'Product' }),
      });
    } catch (error) {
      console.error('Failed to toggle wishlist:', error);
      toast({
        title: t('error'),
        description: t('unableUpdateWishlist'),
        variant: "destructive"
      });
    } finally {
    ...
    const loadProduct = async () => {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          setLoadError(t('productNotFound'));
          toast({
            title: t('error'),
        ...
        setLoadError(t('failedLoadProduct'));
        toast({
          title: t('error'),
          description: t('failedLoadProduct'),
          variant: "destructive",
        });
        navigate('/');
      } finally {
  ...
  const handleAddToCart = useCallback(async (): Promise<boolean> => {
    ...
    if (user?.role === 'vendor') {
      toast({
        ...
        description: t('vendorsCannotCart'),
        variant: "destructive"
      });
      return false;
    }
    ...
    if (user?.role !== 'customer') {
      toast({
        ...
        description: t('onlyCustomersCart'),
        variant: "destructive"
      });
      return false;
    }
    ...
    try {
      ...
      toast({
        title: t('addedToCart'),
        description: t('productAddedToCart', { quantity, name: itemName }),
      });
      return true;
    } catch (error) {
  ...
  const handleReviewSubmit = async () => {
    ...
    if (user?.role !== 'customer') {
      toast({
        title: t('actionNotAllowed'),
        description: t('onlyCustomersReview'),
        variant: "destructive"
      });
      return;
    }
    ...
    if (!reviewComment.trim()) {
      toast({
        title: t('commentRequired'),
        description: t('writeComment'),
        variant: "destructive"
      });
      return;
    }
  ...
  return (
    <div className="relative min-h-screen bg-background">
      {showLoader && (
        <PageLoader
          variant="product"
          title={t('gatheringProductDetails')}
          subtitle={t('pullingLatestInformation')}
          ...
          <div className="container mx-auto px-4 py-8">
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <span className="hover:text-foreground cursor-pointer" onClick={() => navigate('/')}>{t('home')}</span>
                <span>/</span>
                <span className="hover:text-foreground cursor-pointer" onClick={() => navigate('/categories')}>{t(productCategory)}</span>
                <span>/</span>
                  ...
                  {productOriginalPrice && (
                    <div className="absolute top-4 left-4">
                      <Badge className="bg-red-500 text-white">
                        -{Math.round(((productOriginalPrice - productPrice) / productOriginalPrice) * 100)}% {t('off')}
                      </Badge>
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2">
                  {productImages.map((image, index) => (
                    <button
                      ...
                      className={`w-20 h-20 rounded-lg overflow-hidden border-2 flex-shrink-0 ${selectedImage === index ? 'border-primary' : 'border-gray-200 hover:border-gray-300'
                        }`}
                    >
                      <img
                        src={image}
              ...
              <div className="space-y-6">
                    ...
                    <div className="flex items-center">
                      <div className="flex">
                        {[...Array(5)].map((_, i) => (
                          <Star
                            key={i}
                            className={`h-4 w-4 ${i < Math.floor(reviewStats.averageRating || productRating)
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`font-medium ${productStock > 10 ? 'text-green-600' : productStock > 0 ? 'text-orange-600' : 'text-red-600'}`}>
                    {productStock > 0 ? t('inStock', { count: productStock }) : t('outOfStock')}
                  </span>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-4">
                    <Button
                      className={`flex-1 ${!isAuthenticated
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div>
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-xl font-semibold">{t('productDescription')}</h3>
                        {isVendor && (
                          <Button
                            variant="outline"
                            size="sm"
                            ...
                            <div>
                              ...
                              <div className="flex gap-1 mt-2">
                                {[1, 2, 3, 4, 5].map((rating) => (
                                  ...
                                  >
                                    <Star
                                      className={`h-6 w-6 ${rating <= reviewRating
                                        ? 'fill-yellow-400 text-yellow-400'
                          ...
                          <div className="text-center">
                            ...
                            <div className="flex justify-center mb-2">
                              {[...Array(5)].map((_, i) => (
                                <Star
                                  key={i}
                                  className={`h-5 w-5 ${i < Math.floor(reviewStats.averageRating)
                                    ? 'fill-yellow-400 text-yellow-400'
                                    : 'text-gray-300'
                                    }`}
                                />
            ...
            <div className="mb-12">
              <div className="flex items-center justify-between mb-6">
                ...
                >
                  {t('viewAllInCategory', { category: t(productCategory) })}
                </Button>
              </div>
              {isLoadingRelated ? (
                <div className="flex justify-center items-center py-8">
                  <Loader2 className="h-6 w-6 animate-spin text-primary" />
                  <span className="ml-2 text-muted-foreground">{t('loadingRelatedProducts')}</span>
                </div>
            ...
            {/* Recently Viewed */}
            <div className="mb-12">
              <h3 className="text-2xl font-bold mb-6">{t('recentlyViewed')}</h3>
              {relatedProducts.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
